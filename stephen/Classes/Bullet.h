...
public:
	...
	static Bullet* create(HeroTank* owner, float distance, int attackValue,Vec2 pos, BulletType bulletType); //创建子弹指针，确定其所有者，攻击距离，伤害值，子弹类型
	void move(Point destPos); //子弹移动的动作
	Vec2 setDirection(Point curPos);  //子弹移动的方向  获得一个点
	bool Bullet::init(HeroTank* owner, float distance, int attackValue,Vec2 pos, BulletType bulletType ); //初始化子弹
	
private:
	
	HeroTank* _owner;//子弹所属的坦克名字
	Vec2 _pos; //子弹的位置
